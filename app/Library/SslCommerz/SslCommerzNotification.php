                ...
                if ($status == "VALID" || $status == "VALIDATED") {
                    if ($merchant_trans_currency == "BDT") {
                        if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $amount) < 1)) {
                            ...
                            $this->error = "Transaction data mismatch or tempered.";
                    ...
                    $this->error = "Failed transaction. Status: " . $status;
                    return ['status' => false, 'error' => $this->error];
                }
            } else {
                $this->error = "Failed to connect with SSLCOMMERZ. HTTP Code: " . $code;
                return ['status' => false, 'error' => $this->error];
            }
        } else {
            $this->error = "Invalid data. Merchant transaction ID or amount is missing.";
            return ['status' => false, 'error' => $this->error];
        }
    }
